...
struct BaseExtensionManagerConstructorArgs {
    address owner;
 ...
 */
contract BaseExtensionManager is IExtensionManager {
    ...
    constructor(BaseExtensionManagerConstructorArgs memory args) {
    ...
    function _installExtensions(string[] memory names) internal {
        for (uint256 i = 0; i < names.length; i++) {
            ...
            string memory err = string(abi.encodePacked("Extension not found: ", names[i]));
            require(address(ext) != address(0), err);
            LibExtensions.installExtension(IExtension(ext));
        }
    }

    function getExtensions() external view override returns (IExtension[] memory) {
        return LibExtensions.load().installed;
    }
