...
contract RegistrarRemovalExt is BaseEntityRemovalExt {

    modifier onlyAdmin {
        require(LibAccess.isAdmin(msg.sender), "RegistrarEntityRemovalExt: restricted to admins");
        _;
    }
    ...
    function metadata() external pure override returns (ExtensionMetadata memory) {
        return ExtensionMetadata({
            name: LibExtensionNames.REGISTRAR_ENTITY_REMOVAL,
            version: Version(1,0)
        });
    }
     ...
     */
    function deactivateEntity(IRemovableEntity entity, string calldata reason) external onlyAdmin {
     ...
     */
    function reactivateEntity(IRemovableEntity entity) external onlyAdmin {
     ...
     */
    function removeEntity(IRemovableEntity entity, string calldata reason) external onlyAdmin   {
